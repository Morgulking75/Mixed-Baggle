...
export class HomeComponent {
	...
	public generateBoard = () => {
		...
		this.shakenDice = [];
		const availableLetters = this.settings.dice.dice.slice();
		const shakerService = new ShakerService(this.settings.seed);
		const letters = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"];

		for (let x = 0; x < this.settings.height; x++) {
			const col = new Array<Die>();

			for (let y = 0; y < this.settings.width; y++) {
				if (availableLetters.length > 0) {
					const chosenDie = shakerService.chooseDie(availableLetters);
					...
					const split = chosenDie.showing.split("");

					split.forEach(character => {
						const charIndex = letters.indexOf(character);
		...
		const traverse = new TraverseService(hashTable, this.shakenDice);
	...
	public showOnBoard = () => {
		const word = [this.enteredText];

		const traverse = new TraverseService(this.hashWordList(word), this.shakenDice);
	...
	public onKey = ($event) => {
		if (!this.guessedWords[this.enteredText.toUpperCase()]
			...
			const wordLength = this.enteredText.length > 8 ? 8 : this.enteredText.length;
			const points = this.scoring[wordLength];
	...
	private hashWordList(wordList: Array<string>): Map<string, HashEnum> {
		const hashTable = new Map<string, HashEnum>();
